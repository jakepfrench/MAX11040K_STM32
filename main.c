...
#include "MAX11040K.h"
#include <inttypes.h>
volatile uint32_t adc_ready_count = 0;
...
int main(void)
{
  ...
  uint8_t sampling[4] = {0x00, 0x00, 0x00, 0x00};
  uint8_t drt[2]      = {0xC0, 0x00};
  uint8_t cfg         = 0x30; //0x18;
  uint8_t error       = 0x01;
  ...
  while(error)
  {
	  ...
	  if( HAL_GPIO_ReadPin(ADC_FAULT_GPIO_Port, ADC_FAULT_Pin) )
		  error = 0x00;
  }

  while (1)
  {
      if (adc_ready_count > 0)
      {
          __disable_irq();
          adc_ready_count--;
          __enable_irq();
          ...
          //ch = MAX11040K_value(payload, MAX11040K_CH0);
          //val = MAX11040K_to_voltage(ch);
      }
  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin : ADC_FAULT_Pin */
  GPIO_InitStruct.Pin = ADC_FAULT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(ADC_FAULT_GPIO_Port, &GPIO_InitStruct);
